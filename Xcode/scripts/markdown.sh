...
mkdir -p "$(dirname -- "$dst")"
# xcodebuild inherits the invoking shell's environment, which bin/build's own
# `env -u ...` sanitizes before ninja ever runs -- mirrored here so this
# script phase doesn't depend on the invoker having remembered the same
# thing.
