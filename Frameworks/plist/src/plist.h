...
namespace plist
{
	...
	// Replaces boost::make_recursive_variant<…>::type. The recursion (any_t
	// containing array_t/dictionary_t containing any_t) works without an
	// equivalent to boost::recursive_wrapper because std::vector/std::map
	// support incomplete element types (C++17), so array_t/dictionary_t can
	// be named here while any_t is still incomplete.
	struct any_t
	{
		...
		// boost::variant::empty() unconditionally returns false (verified
		// against the actual installed variant.hpp header before removing
		// it) — it only ever guarded against boost's transient exception-safety
		// backup state, never against holding a default-constructed value.
		// plist::is_true()'s one caller of empty() therefore always visited
		// the variant regardless of this check; this reproduces the same
		// outcome without a dead branch, since the default alternative
		// (bool, false) already converts to `flag = false` via the identity
		// conversion in convert_to_helper_t either way.
	...
	// Drop-in replacements for boost::get<T>(variant) / boost::get<T>(&variant).
