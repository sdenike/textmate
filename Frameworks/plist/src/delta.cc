...
static void delta_plist_helper (plist::dictionary_t const& oldDict, plist::dictionary_t const& newDict, plist::dictionary_t& changed, plist::array_t& deleted, std::vector<std::string> path = std::vector<std::string>())
{
	...
	for(auto const& newValue : newDict)
	{
		...
		// Qualified: any_t is now a real type in namespace plist (not a
		// boost::variant alias), so ADL makes the file-local equal() above
		// ambiguous with plist::equal() here otherwise. Both perform the
		// same deep structural comparison (verified by reading both
		// implementations), so this is a disambiguation, not a behaviour
		// change.
